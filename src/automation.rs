...
//! a browser tab, quit. On macOS this is an Apple Event sent through
//! `osascript`, the same thing ⌘W or ⌘Q does, so the app gets to prompt or
//! refuse. No other platform has an implementation yet; the verbs say so
//! instead of falling back to a signal, because a signal is not a polite
//! request.
//!
