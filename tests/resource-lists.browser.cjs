...
(async () => {
  ...
  try {
    ...
    await table.locator('[data-list-group="stale"]').click();
    await table.locator('[data-list-group="recent"]').click();
    const tableTopBeforeSelection = (await table.boundingBox()).y;
    await rows.first().locator('.row-title').click();
    assert.equal((await table.boundingBox()).y, tableTopBeforeSelection, 'batch actions must not shift the table');
