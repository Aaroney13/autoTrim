...
(async () => {
  ...
  try {
    ...
    // Reviews keep exact frozen targets and exclude pinned/active tabs.
    ...
    await list.locator('[data-list-review]').click(); assert.equal(await page.locator('.review-target').count(), 40);
    ...
    await list.locator('[data-list-review]').click(); assert.equal(await page.locator('.review-target').count(), 78);
    await page.locator('#review-cancel').click(); await clear();
    await rows.first().locator('input').check();
