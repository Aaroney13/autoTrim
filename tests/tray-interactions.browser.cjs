...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      state.snap = {taken_at:epochNow(),system:{total_mem:16*GB,used_mem:8*GB,used_swap:0,total_swap:GB,uptime_secs:1000,cpu_pct:5,load_one:1,load_five:1,load_fifteen:1},groups:[],browsers:[],sessions:[],advice:[],ports:[port(101,3000,'Frontend server'),port(102,8080,'API server'),port(103,9000,'Managed service',true)],trends:[trend('g:Chrome','Google Chrome',3*GB),trend('g:Codex','Codex sessions',2*GB)]};
      state.src={daemon_running:true,interval_secs:30,snapshot_taken_at:epochNow()};
      window.__TAURI__={core:{invoke:async command=>({snapshot:state.snap,source_info:state.src,action_log:[],settings:null,service_info:null})[command]}};
      renderAll(true);
    });
